...
---------------------- INPUT FILES ---------------------------------------------
"NRELOffshrBsline5MW_Onshore_ElastoDyn_no_gen_dof_no_tower.dat"    EDFile          - Name of file containing ElastoDyn input parameters (quoted string)
...
False         CalcSteady      - Calculate a steady-state periodic operating point before linearization? [unused if Linearize=False] (flag)
      ...
      0.001   TrimTol         - Tolerance for the rotational speed convergence [used only if CalcSteady=True] (-)
       0.01   TrimGain        - Proportional gain for the rotational speed error (>0) [used only if CalcSteady=True] (rad/(rad/s) for yaw or pitch; Nm/(rad/s) for torque)
          ...
          500.000, 500.561, 501.121, 501.682, 502.242, 502.803, 503.363, 503.924, 504.484, 505.045, 505.605, 506.166   LinTimes        - List of times at which to linearize (s) [1 to NLinTimes] [used only when Linearize=True and CalcSteady=False]
